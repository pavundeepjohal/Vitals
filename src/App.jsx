...
function keys(user) {
  ...
  return {
    data:      `vitals-data-${u}`,
    exercises: `vitals-exercises-${u}`,
    habits:    `vitals-habits-${u}`,
  };
}
...
function loadUser(user) {
  ...
  try {
    ...
    const e = localStorage.getItem(k.exercises);
    const h = localStorage.getItem(k.habits);
    return {
      data:      d ? JSON.parse(d) : {},
      exercises: e ? JSON.parse(e) : null,
      habits:    h ? JSON.parse(h) : null,
    };
  } catch { return { data:{}, exercises:null, habits:null }; }
}
    ...
    <div style={S.panel}>
      ...
      <section style={S.section}>
        ...
        {(()=>{
          ...
          if(points.length===0) return <p style={S.empty}>No metrics logged yet — fill them in on any day.</p>;
          return (
...
// ─── SETTINGS ───
function SettingsView({ habits, exercises, users, currentUser, saveHabits, saveExercises, saveData, saveUsers, setCurrentUser }) {
...
function UserPicker({ users, currentUser, onSelect }) {
  ...
  return (
    <div style={S.pickerWrap} ref={el => ref.current = el}>
...
export default function HealthTracker() {
  ...
  useEffect(()=>{
    try {
      const u=localStorage.getItem("vitals-users");
      const cu=localStorage.getItem("vitals-current-user");
      if (u) { const parsed=JSON.parse(u); setUsers(parsed); }
      if (cu) setCurrentUser(JSON.parse(cu));
    } catch {}
  },[]);

  // Load per-user data whenever currentUser changes
  useEffect(()=>{
    const s=loadUser(currentUser);
    setData(s.data);
    if (s.exercises&&s.exercises.length>0) setExercises(s.exercises);
    else { setExercises(DEFAULT_EXERCISES); persist(keys(currentUser).exercises, DEFAULT_EXERCISES); }
    if (s.habits&&s.habits.length>0) setHabits(s.habits);
    else { setHabits(DEFAULT_HABITS); persist(keys(currentUser).habits, DEFAULT_HABITS); }
    setSelectedDay(null);
  },[currentUser]);

  const saveData      = useCallback((d)=>{ setData(d);      persist(keys(currentUser).data,d);      },[currentUser]);
  const saveExercises = useCallback((e)=>{ setExercises(e); persist(keys(currentUser).exercises,e); },[currentUser]);
  const saveHabits    = useCallback((h)=>{ setHabits(h);    persist(keys(currentUser).habits,h);    },[currentUser]);

  function saveUsers(list) {
    setUsers(list);
    persist("vitals-users", list);
  }
  function switchUser(u) {
    setCurrentUser(u);
    persist("vitals-current-user", u);
  }
  ...
  function handleFoodSelect(dayKey,groupKey,optionId) {
    ...
    saveData({...data,[dayKey]:{...dd,food}});
  }
  function handleHabitToggle(dayKey,habitId) {
    const dd=getDayData(dayKey);
    saveData({...data,[dayKey]:{...dd,habits:{...(dd.habits||{}),[habitId]:!dd.habits?.[habitId]}}});
  }
  function handleExToggle(dayKey,ex) {
    const dd=getDayData(dayKey);
    const cur=dd.exercises||[];
    saveData({...data,[dayKey]:{...dd,exercises:cur.includes(ex)?cur.filter(e=>e!==ex):[...cur,ex]}});
  }
  function handleNoteChange(dayKey,val) {
    const dd=getDayData(dayKey);
    saveData({...data,[dayKey]:{...dd,notes:val}});
  }
  function handleMetricChange(dayKey,field,val) {
    const dd=getDayData(dayKey);
    const metrics={...(dd.metrics||{}),[field]:val};
    saveData({...data,[dayKey]:{...dd,metrics}});
  }
  ...
  return (
    <div style={S.root}>
      ...
      <main style={S.main}>
        ...
        {view==="settings" && <SettingsView habits={habits} exercises={exercises} users={users} currentUser={currentUser} saveHabits={saveHabits} saveExercises={saveExercises} saveData={saveData} saveUsers={saveUsers} setCurrentUser={switchUser}/>}
      </main>
      <nav style={S.nav}>
